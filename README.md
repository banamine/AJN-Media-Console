...
This repo has two active deployment surfaces that are **not equivalent** — only one of them runs the full application.

| Target | What runs | Backend (`server.ts` / `/api/*`) |
| :--- | :--- | :--- |
| **Self-hosted / Cloud Run** (`ajn-archive-iptv-player.ai.studio`, `the-liberty-freedom.ai.studio`, or a `npm run start` container) | The full app — Vite frontend **and** the Express backend on Port 3000 | ✅ Available |
| **GitHub Pages** (`banamine.github.io/AJN-Media-Console/`, via [`.github/workflows/deploy.yml`](./.github/workflows/deploy.yml)) | Static frontend only (`npm run build` output, no server process) | ❌ Not available — Pages serves static files only |

**Why this matters:** `deploy.yml` runs the same `npm run build` that produces the self-hosted app's `dist/` — there's no separate static-only build target that strips backend-dependent features. That means on Pages, anything routed through the Express layer (`/api/stream-proxy`, Rumble ingestion, `/api/sync/push`, the podcast tuner API, news harvest) is structurally dead, not just occasionally broken. M3U/IPTV imports and similar proxy-routed streams will fail to load there by design.

**Investigation note (2026-08):** a base-path bug (`/ajn-liberty-broadcast/` baked into a build served at `/AJN-Media-Console/`) was traced and fixed in PR #1, and the Pages deploy was confirmed live and rendering past the boot splash. While verifying, it became clear the Pages deployment itself was likely an **unintentional deployment target** — `deploy.yml` has no customization and reads like default scaffolding from the initial `chore: initialize project infrastructure` commit, no "live app" reference in project history points at Pages, and this codebase already has two purpose-built static-site systems (the Nexus-TV-O static player generator, and the separate `Classic-TV-Guide-Video-Player` repo) for exactly the no-backend use case Pages would otherwise serve. Rather than disable the Pages workflow outright, the client now detects the static build (`VITE_STATIC_DEMO=true`, set only in `deploy.yml`) and shows a dismissible in-app banner clarifying that proxy-routed features won't work on this build, so a visitor doesn't mistake the limited channel list for a regression.

If you need a genuinely static, backend-free deployment, prefer Nexus-TV-O or `Classic-TV-Guide-Video-Player` over enabling Pages here.
